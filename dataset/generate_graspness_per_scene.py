...
if __name__ == '__main__':
    ...
    save_path_root = os.path.join(dataset_root, 'graspness_per_scene')
    ...
    for scene_id in range(0, 100):
        ...
        for ann_id in range(1):
            ...
            graspness_res = dict()
            graspness_res['grasp_points'] = grasp_points
            
            max_graspness = np.max(grasp_points_graspness)
            min_graspness = np.min(grasp_points_graspness)
            grasp_points_graspness = (grasp_points_graspness - min_graspness) / (max_graspness - min_graspness)
            graspness_res['graspness'] = grasp_points_graspness
            np.save(os.path.join(save_path, str(ann_id).zfill(4) + '.npy'), graspness_res)
